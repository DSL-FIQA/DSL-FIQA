...
class Image(torch.utils.data.Dataset):
    def __init__(self, image_path, dataset, transform, num_crops=20, num_landmark=500, normalization=True, img_size=224):
        ...
        self.landmark = np.load(image_path.replace('img', 'landmark').replace('.png', '.npy'))
...
if __name__ == '__main__':
    ...
    # config file
    path_data = './dataset/custom/img'

    filenames = glob(os.path.join(path_data, '*.png'))+glob(os.path.join(path_data, '*.jpg'))
    print('Number of testing images: {}'.format(len(filenames)))
    ...
    ckpt_path_DE = os.path.join('./ckpt/custom/DE.pt')  
    ckpt_path_net = os.path.join('./ckpt/custom/IQA.pt')  
    ...
    print('Predict Custom Data')
    for i, image_path in enumerate(tqdm(filenames)):               
        # data load
    ...
    # 創建一個 DataFrame
    df = pd.DataFrame({'Name': all_name, 'Predict': all_pred})
    # 將 DataFrame 存為 CSV 檔
    save_dir = os.path.join("./result/custom")
